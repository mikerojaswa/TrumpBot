...
class Bot {
    ...
    static checkMessage(message) {
        ...
        if (messageText && botRegex.test(messageText)) {
            ...
            request('https://api.whatdoestrumpthink.com/api/v1/quotes/random', function (error, response, data) {
                var cleanedUpData = JSON.parse(data);
                console.log(cleanedUpData.message);
                const messageText = cleanedUpData.message;
